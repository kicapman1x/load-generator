...
def bootstrap():
    ...
    global facial_dir, facial_api, rmq_url, rmq_port, rmq_username, rmq_password, ca_cert, secret_key, mysql_url, mysql_port, mysql_user, mysql_password, CONSUME_QUEUE_NAME, logdir, loglvl, mysql_db_s3
    ...
    mysql_db_s3 = os.environ.get("MYSQL_DB_SATELLITE1")
    CONSUME_QUEUE_NAME = "ingest_facial_data_s3"
    ...
    logging.basicConfig(
        filename=f'{logdir}/satellite3.log',
        level=log_level,
...
def process_message(ch, method, properties, body):
    ...
    try:
        ...
        conn_s3.commit()
        logger.info(f"[{trace_id}] Successfully commited data for passenger: {p_key} into satellite 3 database.")
...
def insert_full_data_satellite3(conn, passenger_key, trace_id, facial_image, departure_date, arrival_airport):
    cursor = conn.cursor()
    insert_query = """
    ...
    logger.info(f"[{trace_id}] Inserted data for passenger {passenger_key} into satellite 3 database.")
